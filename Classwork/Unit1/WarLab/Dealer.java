...
public abstract class Dealer {
    ...
    private static boolean isOpenGame = false;
    ...
    /**
     * Sets the game mode to "open game", where players can see each other's
     * cards.
     * This affects how cards are displayed during gameplay.
     */
    public static void setOpenGame() {
        isOpenGame = true;
    }
    ...
    public static void PrintBothCards(Stack<Card> player1Deck, Stack<Card> player2Deck) {
        ...
        // Copy player1Deck to tempStack1 to iterate through it
        while (!player1Deck.isEmpty()) {
            tempStack1.push(player1Deck.pop());
        }
        while (!tempStack1.isEmpty()) {
            Card card = tempStack1.pop();
            player1Deck.push(card);
            tempStack2.push(card);
        }

        int cardCount1 = 0;
        while (!tempStack2.isEmpty()) {
            Card card = tempStack2.pop();
            if (isOpenGame) {
                currentLine.append("[" + card.rank + "]");
            } else {
                currentLine.append("[?]");
            }
            cardCount1++;
            if (cardCount1 % 5 == 0 && !tempStack2.isEmpty()) {
        ...
        currentLine = new StringBuilder();
        tempStack1 = new Stack<Card>();
        tempStack2 = new Stack<Card>();

        // Copy player2Deck to tempStack1 to iterate through it
        while (!player2Deck.isEmpty()) {
            tempStack1.push(player2Deck.pop());
        }
        while (!tempStack1.isEmpty()) {
            Card card = tempStack1.pop();
        ...
        while (!tempStack2.isEmpty()) {
            ...
            cardCount2++;
            if (cardCount2 % 5 == 0 && !tempStack2.isEmpty()) {
