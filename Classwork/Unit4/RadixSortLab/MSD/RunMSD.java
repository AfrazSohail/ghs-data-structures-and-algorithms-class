/**
 * Manages the GUI and visualization for the MSD (Most Significant Digit) radix sort.
 * Creates the canvas, initializes visual elements (pointer, number line, bucket markers, animal images),
 * and provides utility methods for animation and rendering. Unlike LSD, MSD includes visual bucket
 * boundaries to show recursive subarray partitions.
 *
 * @author Afraz Sohail
 * @version 1.0
 * Written by AI
...
public class RunMSD {
    ...
    /** JLabel marking the left boundary of the current subarray bucket */
    public static JLabel bucketLeft = new JLabel();
    /** JLabel marking the right boundary of the current subarray bucket */
    public static JLabel bucketRight = new JLabel();
    ...
    /**
     * Initializes and runs the MSD sort visualization.
     ...
     */
    public static void runMSDSort(AnimalVal[] vals) {
        JFrame frame = new JFrame("");
        frame.setSize(1250, 350 + 10 + 35);
        ...
        new Thread(() -> {
            MSDSort.msdSort(animVals);
        }).start();
    }

    /**
     * Initializes helper visual elements including the pointer sprite,
     * the number line, the bucket boundary markers, and the speed control slider.
     */
    private static void initHelpers() {
        ...
        String bucketLeftPath = "Classwork\\Unit4\\RadixSortLab\\Data\\Helper\\bucketLeft.png";
        String bucketRightPath = "Classwork\\Unit4\\RadixSortLab\\Data\\Helper\\bucketRight.png";
        ...
        numberLine.setBounds(50 + 36 * 3, 100, 360 * 3, 28 * 3);
        canvas.add(numberLine);

        ImageIcon bucketLeftIcon = new ImageIcon(bucketLeftPath);
        Image scaledBucketLeft = bucketLeftIcon.getImage().getScaledInstance(48, 48, Image.SCALE_SMOOTH);
        bucketLeft = new JLabel(new ImageIcon(scaledBucketLeft));
        bucketLeft.setBounds(50 - 24, 250, 48, 48);
        canvas.add(bucketLeft);
        bucketLeft.setVisible(false);

        ImageIcon bucketRightIcon = new ImageIcon(bucketRightPath);
        Image scaledBucketRight = bucketRightIcon.getImage().getScaledInstance(48, 48, Image.SCALE_SMOOTH);
        bucketRight = new JLabel(new ImageIcon(scaledBucketRight));
        bucketRight.setBounds(50 + 360 * 3 - 24, 250, 48, 48);
        canvas.add(bucketRight);
        bucketRight.setVisible(false);
